...
sf::Texture loadTexture(std::string path) {
    ...
    if(!texture.loadFromFile(path)) {
        std::cout << "Error Loading Texure" << std::endl;
        exit(1);
    }
    return texture;
}
...
int main(){
    ...
    sf::Texture wolf_tex { loadTexture("lobo.png") };
    sf::Texture little_hat_tex { loadTexture("chapeuzinho.png") };
    sf::Texture grass_tex { loadTexture("floresta.jpg") };

    const int STEP {100};

    Entity wolf(1, 1, STEP, wolf_tex);
    Entity little_hat(3, 3, STEP, little_hat_tex);
    ...
    while (window.isOpen()) {
        ...
        while (window.pollEvent(event)) {
            if(event.type == sf::Event::Closed){
                ...
                moveEntity(event.key.code, little_hat, {sf::Keyboard::A, sf::Keyboard::W, sf::Keyboard::D, sf::Keyboard::S });
            }
        }
